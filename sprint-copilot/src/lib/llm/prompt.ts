...
export function buildClassificationPrompt(
  ...
  const system = [
    ...
    '  string "null".',
    "- matches_sprint_focus: true only if a sprint focus is given below AND this",
    "  issue clearly relates to it — e.g. focus says \"LLM features\" and this issue",
    "  is genuinely about adding or improving an AI/LLM capability, not just",
    "  loosely in the same area. False if no sprint focus is given below, or the",
    "  issue doesn't clearly relate to it. This directly changes which issues get",
    "  selected for the sprint (matching issues are preferred within their",
    "  bucket, ahead of smaller non-matching ones), so don't set it true for a",
    "  vibes-based connection.",
    "Classify every issue given — do not skip or invent any.",
    ...(sprintFocus?.trim()
      ? [
          "",
          "The user has also given this sprint's focus — use it to set",
          "matches_sprint_focus per issue as instructed above, and let it shift how",
          "you judge effort/urgency, but never override the",
